    ...
    public class PropertiesChecker
    {
        ...
        public static bool CheckProperties<V>(Dictionary<string, V> properties)
        {
            ...
            foreach(KeyValuePair<string, V> kv in properties) 
            {
                ...
                {
                    if(Log.GetEnable()) Log.w("Incorrect property - property values must be one of: String, Numberic, Boolean, DateTime, Array, Row");
                    ret = false;
                }
        ...
        public static bool CheckProperties(List<object> properties)
        {
            ...
            foreach (object value in properties)
            {
                ...
                {
                    if(Log.GetEnable()) Log.w("Incorrect property - property values in list must be one of: String, Numberic, Boolean, DateTime, Row");
                    ret = false;
                }
        ...
        public static bool CheckProperties(string properties) 
        {
            if (properties is string && System.Text.Encoding.UTF8.GetBytes(Convert.ToString(properties)).Length > 2048) {
                if(Log.GetEnable()) Log.w("Incorrect properties - the string is too long: " + (string)(object)properties);
                return false;
            }
            return true;
        }
        public static bool CheckProperties(double properties) 
        {
            ...
            {
                if(Log.GetEnable()) Log.w("Incorrect properties - number value is invalid: " + properties + ", the data range is -9E15 to 9E15, with a maximum of 3 decimal places");
                return false;
            }
            return true;
        }
        public static bool CheckString(string eventName)
        {
            ...
            {
                if(Log.GetEnable()) Log.w("Incorrect event name - the string is null");
                return false;
            }
            ...
            else
            {
                ...
                if(Log.GetEnable()) Log.w("Incorrect event name - the string is invalid for Insight: " + eventName + ", event name and properties name rules: must be character string type, starting with a character and containing figures, characters, and an underline \"_\", with a maximum length of 50 characters");
                return false;
            }
        }
