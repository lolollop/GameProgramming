...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("The lowest possible spawn delay after difficulty scaling")]
    public float minimumSpawnDelay = 0.6f;
    [Tooltip("How much faster enemies spawn at higher difficulty")]
    public float spawnDelayDifficultyScale = 0.08f;
    ...
    private void CheckSpawnTimer()
    {
        ...
        float currentDelay = GetScaledSpawnDelay();
        if (Time.timeSinceLevelLoad > lastSpawnTime + currentDelay && (currentlySpawned < maxSpawn || spawnInfinite))
        {
    ...
    private void SpawnEnemy(Vector3 spawnLocation)
    {
        ...
        if (enemyPrefab != null)
        {
            ...
            if (enemy != null)
            {
                ...
                enemy.moveSpeed *= GetEnemySpeedMultiplier();
            }
    ...
    private float GetScaledSpawnDelay()
    {
        float difficultyLevel = DifficultyDirector.CurrentDifficultyLevel;
        return Mathf.Max(minimumSpawnDelay, baseSpawnDelay - difficultyLevel * spawnDelayDifficultyScale);
    }

    private float GetEnemySpeedMultiplier()
    {
        return 1f + DifficultyDirector.CurrentDifficultyLevel * 0.07f;
    }
}
